...
Int_t GEMCluster::GetClusterTimeBin() {  
  ...
  for (int i = 0; i < nbofhits; i++) {	
    //    vector< Float_t > timeBinADCs = ((GEMHit*)temp[i])->GetTimeBinADCs() ;
...
void GEMCluster::ClusterPositionPulseHeghtWeight() {  // Calculate the fposition and the total fClusterSumADCs
  float hitposition, q;
  TObjArray &temp = *fArrayOfHits;
  Int_t nbofhits =  GetNbOfHits() ;
  for (int i = 0; i < nbofhits; i++) {	
   q  = ((GEMHit*)temp[i])->GetHitADCs() ;
   //if(q>1000)cout<<"####### adc form hit: "<<q<<endl;
...
Bool_t GEMCluster::IsGoodCluster() {
  ...
  //  printf("   GEMCluster::IsGoodCluster => Before tests: nb of hits = %d\n", fNbOfHits) ;
  if ( (fNbOfHits > fMaxClusterSize) || (fNbOfHits < fMinClusterSize) ) {
    ClearArrayOfHits() ;
    fIsGoodCluster = kFALSE ;
    fNbOfHits = fArrayOfHits->GetEntries() ;
  } 
  //  printf("   GEMCluster::IsGoodCluster => After  tests: nb of hits = %d\n", fNbOfHits) ;
